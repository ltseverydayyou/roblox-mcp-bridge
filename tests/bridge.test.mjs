...
test("the Windows manager uses the dashboard WebView UI and verified native update paths", () => {
  const host = readFileSync(new URL("../scripts/windows-manager/ManagerHost.cs", import.meta.url), "utf8");
  const managerHtml = readFileSync(new URL("../scripts/windows-manager/manager.html", import.meta.url), "utf8");
  const launcher = readFileSync(new URL("../scripts/create-windows-launcher.ps1", import.meta.url), "utf8");

  assert.match(host, /pull --ff-only/);
  assert.match(host, /install-harnesses\.mjs/);
  assert.match(host, /--update --yes --plain --server-root/);
  assert.match(host, /ReloadBridgeAsync/);
  ...
  assert.match(host, /Windows\.UI\.Notifications\.ToastNotificationManager/);
  assert.match(host, /GetForegroundWindow/);
  assert.match(host, /CheckSourceAsync\(true\)/);
  assert.match(host, /Interval = 5 \* 60 \* 1000/);
  assert.match(host, /Interval = 15 \* 60 \* 1000/);
  ...
  assert.match(managerHtml, /href="dashboard\.css"/);
  assert.match(managerHtml, /class="topbar"/);
  ...
  assert.match(launcher, /Microsoft\.Web\.WebView2\.Core\.dll/);
  assert.match(launcher, /Microsoft\.Web\.WebView2\.WinForms\.dll/);
  assert.match(launcher, /WebView2Loader\.dll/);
  assert.match(launcher, /manager\.html/);
  assert.match(launcher, /dashboard\.css/);
